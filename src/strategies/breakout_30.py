...
class Breakout30Strategy(BaseStrategy):
    """收盘价突破30日新高（首次），且站上MA200——v0.1 原始策略"""

    name = "breakout_30"
    description = "收盘价突破30日新高（首次），且站上MA200"
    ...
    def screen(self, df: pd.DataFrame, target_date: str, stock_names: dict) -> pd.DataFrame:
        ...
        # 标记当日是否突破30日高点
        df['is_breakout'] = df['qfq_close'] > df['high30']
